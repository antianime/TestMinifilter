...
NTSTATUS AddPath(PUNICODE_STRING FilePath)
{
    ...
    if (ps.DosList[DosSub].Head == NULL)
    {
        ...
        {
            KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "Open link End\n"));
    ...
    for (i = FilePath->Length / sizeof(WCHAR) - 1; i > 2; i--)
        if (FilePath->Buffer[i] == L'\\')
            break;
    PVOID ParentDirBuffer = ExAllocatePool2(POOL_FLAG_NON_PAGED, (i - 2) * sizeof(WCHAR), 'DosP' + 1);
    ...
    PVOID NameBuffer = ExAllocatePool2(POOL_FLAG_NON_PAGED, FilePath->Length - (i + 1) * sizeof(WCHAR), 'DosN' + 1);
    if (NameBuffer == NULL)
    {
        KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "AllocFail\n"));
        return STATUS_INSUFFICIENT_RESOURCES;
